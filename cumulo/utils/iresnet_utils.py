...
from cumulo.utils.viz_utils import line_plot, scatter_plot, images_plot, line_plot_per_dim, line_plot_mean
...
def train(model, optimizer, epoch, lr, trainloader, viz, train_log, class_weights, use_cuda=False,
          classification_weight=1, nb_classes=8):
    model.train()
    ...
    for batch_idx, (_, inputs, *_, labels) in enumerate(trainloader):
        ...
        # file logging
        log_dict = {"iter": cur_iter, "loss": loss.item(), "logpz": mean_logpz, "logdet": mean_trace,
                    "logp(y|z)": -mean_entropy.item(), "epoch": epoch}
    ...
    # file logging
    log_dict = {"epoch": epoch, "accuracy per class": accuracy_per_class.tolist(),
                "f1 per class": f1_per_class.tolist(), "objective": objective}
...
def test(model, epoch, testloader, viz, test_log, use_cuda=False, flag="validation", classification_weight=1,
         nb_classes=8):
    model.eval()
    ...
    # file logging
    log_dict = {"recons err": test_rec_err / nb_tiles, "epoch": epoch,
                "accuracy per class": accuracy_per_class.tolist(), "f1 per class": f1_per_class.tolist()}
    test_log.write("{}\n".format(json.dumps(log_dict)))
    test_log.flush()
