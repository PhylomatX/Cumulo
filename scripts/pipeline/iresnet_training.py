...
from cumulo.models.iresnet import multiscale_conv_iResNet
...
train_dataset = CumuloDataset(os.path.join(root_dir, "label/"), normalizer=normalizer,
                              indices=train_idx)  # change to labelled dir
val_dataset = CumuloDataset(os.path.join(root_dir, "label/"), "npz", normalizer=normalizer,
                            indices=val_idx)  # change to labelled dir
test_dataset = CumuloDataset(os.path.join(root_dir, "label/"), "npz", normalizer=normalizer,
                             indices=test_idx)  # change to labelled dir
...
valloader = torch.utils.data.DataLoader(val_dataset, batch_size=40, collate_fn=tile_collate, shuffle=False,
                                        num_workers=30)
testloader = torch.utils.data.DataLoader(test_dataset, batch_size=40, collate_fn=tile_collate, shuffle=False,
                                         num_workers=30)

# create new one and initialize it
model = multiscale_conv_iResNet(in_shape, nb_blocks, nb_strides, nb_channels, False, inj_pad, coeff, nb_classes,
...
try:
    ...
    for epoch in range(1, 1 + nb_epochs):
        ...
        train_cm, train_acc = train(model, optimizer, epoch, lr, trainloader, viz, train_log, class_weights, use_cuda,
        ...
        if val_acc > best_accuracy or val_f1 > best_f1:
            save_model(model, optimizer, train_cm, val_cm, state, save_dir_best, epoch=epoch, train_accuracy=train_acc,
...
save_model(model, optimizer, train_cm, val_cm, state, save_dir_last, epoch=epoch, train_accuracy=train_acc,
           val_accuracy=val_acc)
